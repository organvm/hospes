...
| 5 | Per-show outreach templates registry | `config/outreach_templates.yaml` externalizes the producer-cold, prior-collaborator, and Ari-note templates; `hospes/drafts.py` validates and loads the tracked version-1 contract | Tenant/show-keyed template selection and revision history so operators can customize safe templates without editing Python while retaining Voice Constitution validation | `config/outreach_templates.yaml`; `hospes/drafts.py`; `docs/guest-operations-os.md` §2 Booking Desk |
